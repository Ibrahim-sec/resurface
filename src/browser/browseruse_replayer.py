...
class BrowserUseReplayer:
    ...
    def _build_hunt_prompt(self, target_url: str, vuln_types: list[str], stop_on_find: bool = False) -> str:
        ...
        stop_instruction = (
            "\n\nIMPORTANT: Once you FULLY EXPLOIT and report the FIRST confirmed vulnerability, "
        ...
        return (
            ...
            "vulnerabilities — you must FULLY EXPLOIT them to prove real impact.\n\n"
            f"## Target\n{target_url}\n\n"
            ...
            "### Phase 3: FULL EXPLOITATION (critical — do not skip)\n"
            "Once you confirm a vulnerability exists, you MUST escalate to full exploitation:\n\n"
            "**SQL Injection:**\n"
            "  1. Confirm injection with a simple tautology (e.g. `' OR 1=1--`)\n"
            "  2. Determine the number of columns: try `' ORDER BY 1--`, `' ORDER BY 2--`, etc. until error\n"
            "  3. Find displayable columns: `' UNION SELECT NULL,NULL,...--` (match column count), "
            "replace NULLs one at a time with a string like `'abc'` to see which appear on page\n"
            "  4. Extract data through displayable columns:\n"
            "     - **Database version**: Oracle: `SELECT banner FROM v$version`, "
            "MySQL: `SELECT version()`, PostgreSQL: `SELECT version()`, MSSQL: `SELECT @@version`\n"
            "     - **Tables**: Oracle: `SELECT table_name FROM all_tables`, "
            "Others: `SELECT table_name FROM information_schema.tables`\n"
            "     - **Columns**: Oracle: `SELECT column_name FROM all_tab_columns WHERE table_name='USERS'`, "
            "Others: `SELECT column_name FROM information_schema.columns WHERE table_name='users'`\n"
            "     - **Sensitive data**: Extract usernames, passwords, emails, etc.\n"
            "  5. If the lab/app hints at a specific objective (e.g. 'determine the database version'), "
            "make sure you actually extract and display that data\n"
            "  6. For Oracle: use `FROM dual` for simple queries, string concat with `||`\n"
            "  7. For NULL type mismatches: try `TO_CHAR()` or cast as needed\n\n"
            ...
            "### Phase 4: Report\n"
            "- Only call report_vulnerability AFTER full exploitation with concrete extracted data\n"
            "- Include the exact payload that worked, what data was extracted, and the full impact\n"
            "- A report that just says 'SQLi confirmed with OR 1=1' is INCOMPLETE — "
            "you must show extracted data\n"
            + stop_instruction
        )
